...
import { Code2, Terminal, Play, Bug, ShieldCheck, Zap, Sparkles, Loader2, Copy, Check } from 'lucide-react'
...
export default function CodeReviewPage() {
  ...
  const handleReview = () => {
    if (!codeToReview || codeToReview.trim().length < 5) return
    complete(codeToReview)
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[#020817] p-4 lg:p-8 space-y-6">
      ...
      {/* Cabecera optimizada para ingenieros */}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 flex-1 min-h-0">
        ...
        <div className="flex flex-col space-y-4">
          <div className="flex-1 flex flex-col bg-white/5 border border-white/10 rounded-2xl overflow-hidden focus-within:border-green-500/40 shadow-inner group">
            ...
            <textarea
              className="flex-1 w-full bg-[#030a1c] p-6 text-green-100/90 font-mono text-sm placeholder:text-gray-700 focus:outline-none resize-none leading-relaxed selection:bg-green-500/20 scrollbar-thin scrollbar-thumb-white/10"
              spellCheck="false"
          ...
          <button
            ...
            className="w-full bg-green-600 hover:bg-green-500 disabled:opacity-30 disabled:cursor-not-allowed text-white px-8 py-4 rounded-xl font-bold flex items-center justify-center gap-3 transition-all hover:scale-[1.01] active:transform-none shadow-xl shadow-green-950/20 group uppercase tracking-widest text-xs"
          >
            {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <Play className="h-4 w-4 fill-current group-hover:rotate-12 transition-transform" />}
            {isLoading ? "Analizando Código..." : "Iniciar Revisión"}
          </button>
        </div>

        {/* Auditoría / Panel de Resultados */}
        <div className="flex flex-col bg-[#030a1c]/50 border border-white/5 rounded-2xl overflow-hidden backdrop-blur-sm relative">
          ...
          <div className="p-4 border-b border-white/10 bg-white/[0.02] flex justify-between items-center shrink-0 z-10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-400 text-xs font-bold uppercase tracking-widest text-green-500/80">Audit Report</span>
            </div>
            {completion && (
              <button 
                onClick={copyToClipboard}
                className="text-gray-500 hover:text-green-400 transition-colors flex items-center gap-1.5"
              >
          ...
          <div className="flex-1 p-8 text-gray-300 leading-relaxed scrollbar-thin scrollbar-thumb-white/10 overflow-y-auto font-sans selection:bg-violet-500/30">
            {error && (
              <div className="bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl p-4 text-sm mb-4">
                <span className="font-bold">Error:</span> {error.message || 'La IA no pudo procesar el código.'}
              </div>
            )}
            {!completion && !isLoading ? (
              ...
              <div className="whitespace-pre-wrap prose prose-invert prose-sm max-w-none prose-pre:bg-zinc-900 prose-code:text-green-400 prose-strong:text-white prose-headings:text-white prose-headings:font-bold">
                {completion}
                {isLoading && <div className="mt-4 flex items-center gap-2 text-green-500 text-xs font-mono"><Loader2 className="h-3 w-3 animate-spin"/> Generando insights...</div>}
              </div>
            )}
          </div>
